...
resource "google_cloud_run_v2_service" "app" {
  ...
  template {
    ...
    vpc_access {
      network_interfaces {
        network    = "projects/${var.project_id}/global/networks/${data.terraform_remote_state.mock_gke_env.outputs.vpc_network_name}"
        subnetwork = "projects/${var.project_id}/regions/${data.terraform_remote_state.mock_gke_env.outputs.region}/subnetworks/${data.terraform_remote_state.mock_gke_env.outputs.vpc_subnetwork_name}"
      }
...
data "terraform_remote_state" "mock_gke_env" {
  backend = "local"
  config = {
    path = "../../../../mock-gke-env/terraform.tfstate"
  }
}
